...
def save_spike_data_txt(spike_data, file_path):
    """
    将脉冲数据保存为 TXT 文件，每行一个样本
    :param spike_data: 形状为 (N, T, 784) 的 NumPy 数组
    :param file_path: 目标 TXT 文件路径
    """
    with open(file_path, 'w') as f:
        for sample in spike_data:
            for t in range(sample.shape[0]):  # 遍历时间步
                line = ' '.join(map(str, sample[t]))  # 以空格分隔
                f.write(line + '\n')
            f.write('\n')  # 每个样本之间空一行，便于区分
...
def gen_dataset_spike(train_pics,test_pics,T):
    ...
    # 保存为单个二进制文件
    save_spike_data(all_spikes, "./simulation_spikes.bin")
    # save_spike_data_txt(all_spikes, "./all_spikes.txt")
